<!-- Mobil nav slide ind fra siden
https://www.w3schools.com/howto/howto_js_sidenav.asp

(.sidenav ændre left:0; til right:0;)
https://www.w3schools.com/howto/tryit.asp?filename=tryhow_js_sidenav_full -->

<header>
  <section id="header__top">
    <h1 id="title">Jagt & Fiskerimagasinet</h1>

    <nav id="header__top--nav">
      <ul>
        <li id="nak-btn">NAK ET DYR</li>
        <li><img class="header__top--icon" src="img/icon/indkøbskurv.svg" alt="Indkøbskurv">Indkøbskurv</li>
        <li><img class="header__top--icon" src="img/icon/din-konto.svg" alt="Din konto">Din konto</li>
      </ul>
    </nav>

    <img id="mobil_menu" src="img/icon/mobil-menu.svg" alt="Mobil Menu">
  </section>

  <section id="header__bottom">
    <img id="logo" src="img/logo-dark-bg.svg" alt="Jagt & Fiskerimagasinet">

    <nav id="header__bottom--nav">
      <ul>
        <li>Hjem</li>
        <li>Webshop</li>
        <li>Om os</li>
        <li>Kontakt</li>
      </ul>
    </nav>
  </section>
</header>
